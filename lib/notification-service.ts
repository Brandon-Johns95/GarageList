...
export async function sendNotification({
  ...
  try {
    ...
    // 2. Check if user has push notifications enabled for this type
    ...
    const pushEnabled = preferenceData?.push_enabled ?? true

    // 3. If push is enabled, send push notification
    if (pushEnabled) {
      try {
        ...
        // Continue even if push fails - the in-app notification was already created
      }
    }
